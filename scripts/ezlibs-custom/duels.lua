...
local function _start_summon_anim(pid, st, card, start_x, start_y, start_s, end_x, end_y, end_s, kind, opts)
  ...
  if hidden_set then
    ...
    _alloc_sprite(pid, duels.OPP_HAND_SPRITE_ID, duels.FACE_DOWN_TEX, duels.CARD_ANIM, duels.CARD_STATE)
    sprite_id = duels.OPP_HAND_SPRITE_ID
  else
  ...
  if kind == "set" then
    ...
    if not hidden_set then
      alt_sprite_id = duels.OPP_HAND_SPRITE_ID
...
function _draw_hands(pid, st)
  ...
  do
    ...
    local y_tl = oh.y or 0

    for i = 1, opp_count do
      local x_tl = start_x + (i - 1) * step_x
      local x, y = _apply_card_origin_if_needed(x_tl, y_tl, hsx, hsy)
      local zi = (hz or 0) + i
      _draw_sprite_obj(pid, duels.OPP_HAND_SPRITE_ID, duels.OPP_HAND_OBJ_PREFIX .. i, x, y, hsx, hsy, zi, duels.CARD_STATE, oh.ro)
    end
  ...
  if not (st.in_summon_menu or st.in_field_menu or st.in_pause_menu or st.in_spells_menu or (st.pos_anim and st.pos_anim.active) or (st.attack_anim and st.attack_anim.active)) then
...
function _draw_monsters(pid, st)
  ...
  if pos_side == "opp" then
    ...
    if mz1 and mz1.card then
      local marker = mz1.facedown and duels.FACE_DOWN_TEX or ((mz1.card and mz1.card.tex) or "__missing_tex__")
      ...
      if mz1.facedown then
        _alloc_sprite(pid, duels.OPP_HAND_SPRITE_ID, duels.FACE_DOWN_TEX, duels.CARD_ANIM, duels.CARD_STATE)
        _draw_sprite_obj(pid, duels.OPP_HAND_SPRITE_ID, duels.MZ1_OBJ_ID,
  ...
  if pos_side == "ply" then
    ...
    if mz2 and mz2.card then
      local marker = mz2.facedown and duels.FACE_DOWN_TEX or ((mz2.card and mz2.card.tex) or "__missing_tex__")
      ...
      if mz2.facedown then
        _alloc_sprite(pid, duels.OPP_HAND_SPRITE_ID, duels.FACE_DOWN_TEX, duels.CARD_ANIM, duels.CARD_STATE)
        _draw_sprite_obj(pid, duels.OPP_HAND_SPRITE_ID, duels.MZ2_OBJ_ID,
...
Net:on("tick", function()
  ...
  for pid, st in pairs(st_by_pid) do
    if st then
      ...
      -- ------------------------------------------------------------
      -- 1) Let the opponent AI think/act during opponent turns
      -- ------------------------------------------------------------
