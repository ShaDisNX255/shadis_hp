...
local function load_all_memory()
    return async(function()
        ...
        -- Start all player loads in parallel
        ...
        -- Start all area loads in parallel
...
function ezmemory.get_player_memory(safe_secret)
    ...
    if player_memory[safe_secret] then
        return normalize_player_memory(player_memory[safe_secret])
    else
        player_memory[safe_secret] = normalize_player_memory({
            items = {},
            money = 0,
            fragments = 0,
            tokens = 0,
...
local function get_item_get_duration_seconds(player_id)
    ...
    -- total_duration_ms is usually milliseconds, but guard in case parser returns seconds.
    if duration > 10 then
...
function ezmemory.play_anim_get(player_id)
    return async(function()
        ...
        -- Play HOLD once only.
        -- Do NOT re-send it on a timer; the Navi's own animation file controls its timing.
